// Edge Function: catalog/deactivate-product
// (source: RC4, D10, D12 — EF contracts)
//
// 8-step pattern: CORS → Auth → Input → RPC → Return
// Invokes deactivate_product(JSONB) SECURITY DEFINER RPC
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const parsed = DeactivateProductRequest.safeParse(body);
    if (!parsed.success) {
    ...
    // Step 8: Return EFResult<DeactivateResult>
    return Response.json(
      ok(data as DeactivateResult),
      { headers: corsHeaders },
    );
  } catch (err) {
