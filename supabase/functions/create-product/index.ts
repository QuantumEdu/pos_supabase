// Edge Function: catalog/create-product
// (source: RC4, RC5, D10, D12 — EF contracts)
//
// 8-step pattern: CORS → Auth → Input → RPC → Return
// Invokes create_product_with_variant(JSONB) SECURITY DEFINER RPC

import { corsHeaders } from "../_shared/cors.ts";
import { validateAuth } from "../_shared/auth.ts";
import { ok, fail } from "../_shared/types.ts";
import type { EFResult } from "../_shared/types.ts";
import { CreateProductRequest } from "../_shared/catalog_schemas.ts";
import type { ProductResult } from "../_shared/catalog_schemas.ts";
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Step 8: Return EFResult<ProductResult>
    return Response.json(
      ok(data as ProductResult),
      { headers: corsHeaders },
    );
  } catch (err) {
