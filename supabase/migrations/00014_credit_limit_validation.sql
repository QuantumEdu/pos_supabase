-- Migration: 00014_credit_limit_validation
-- Adds credit_limit to customers and validates it in create_sale_transaction
-- when a credit payment is attempted.
--
-- Changes:
--   1. Adds credit_limit NUMERIC(14,2) column to customers (NULL = unlimited)
--   2. Updates create_sale_transaction to reject credit sales that exceed
--      the customer's credit limit, including outstanding balances.
--   3. Adds pgTAP tests
--   4. Updates seed with credit limits for fixture customers
...
COMMENT ON COLUMN public.customers.credit_limit IS
  'Credit limit for this customer. NULL = unlimited credit. Validated on credit-sale creation against outstanding + new total.';

-- ============================================================================
-- 2. Update create_sale_transaction with credit limit validation
-- ============================================================================
...
DECLARE
  ...
  v_outstanding_total NUMERIC(14,2) := 0;
BEGIN
  ...
  -- Validate customer exists and is active when provided
  ...
  -- Load credit limit for credit sales (NULL = unlimited)
  IF v_customer_id IS NOT NULL AND v_payments @> '[{"payment_method": "credit"}]'::JSONB THEN
  ...
  -- Pre-compute total from items for credit-limit validation before persisting anything
  IF v_credit_limit IS NOT NULL THEN
    SELECT COALESCE(SUM(COALESCE((item->>'line_total')::NUMERIC, 0)), 0)
      INTO v_total
      FROM jsonb_array_elements(v_items) AS item;

    SELECT COALESCE(SUM(cb.remaining_amount), 0) INTO v_outstanding_total
      FROM public.customer_balances cb
     WHERE cb.customer_id = v_customer_id
       AND cb.company_id = v_company_id
       AND cb.status IN ('pending', 'partial');

    IF v_outstanding_total + v_total > v_credit_limit THEN
      RETURN jsonb_build_object(
        'success', false,
        'code', 'VALIDATION_ERROR',
        'message', 'Credit limit exceeded. Outstanding: $' || v_outstanding_total || ', new total: $' || v_total || ', limit: $' || v_credit_limit
      );
    END IF;

    -- Reset v_total — will be recomputed in the item loop with precise values
    v_total := 0;
  END IF;
...
COMMENT ON FUNCTION public.create_sale_transaction(JSONB) IS
  'Creates sales against real customers(company_id,id), validates customer activity, validates credit limit for credit payments, deducts inventory FEFO, and records payments atomically.';
