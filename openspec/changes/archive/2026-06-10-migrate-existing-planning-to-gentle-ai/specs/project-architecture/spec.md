...
<!-- source: plan_1ra §2 (architecture general), §3 (technical model); plan_2da §14 (database strategy) -->
Reproducible local dev via Supabase CLI + Vue 3 + TypeScript as a static client-side SPA build.
- **Clone & setup**: CLI initializes local Supabase, applies migrations, starts backend. (source: plan_1ra §2)
- **Frontend**: Vue 3 app with HMR + Supabase JS SDK connected to local instance; Vue is allowed only as static client-side build output. (source: plan_1ra §2, constitution.md §10)
- **Deploy**: CLI pushes Edge Functions, migrations, RLS policies to remote. (source: plan_1ra §2)

### R8: Test Infrastructure Foundation
<!-- source: constitution.md §10 (consistency before convenience — quality assurance) -->
Vitest + Vue Test Utils MUST be configured before critical logic implementation. Strict TDD currently disabled; MUST re-enable once runner exists.
- No runner → manual verification criteria required in task specs.
- Runner operational → all tasks follow RED-GREEN-REFACTOR.
...
| 2 | `customer_balances` — materialized view, trigger table, or computed | credit-payments-domain | Unresolved |
